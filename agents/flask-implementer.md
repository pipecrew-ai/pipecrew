...
### 1. Orient
Per R1, you've already read the repo's `AGENTS.md` and the agent-context docs it points to. Per R10, find the closest analog in this repo before writing new code — read the OpenAPI spec (api-first) or inline contract (code-first), and 2–3 existing blueprints + their services to absorb the concrete patterns: DI style (app factory + extensions, flask-injector, or manual constructors), validation library, ORM usage, error-handler registration, auth decorators, config loading. If THIS repo has no analog, scan sibling flask repos in the workspace before falling back to plugin anti-patterns.
