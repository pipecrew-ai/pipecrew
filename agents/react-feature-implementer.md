...
Read and apply `{plugin_dir}/docs/implementer-common-rules.md` (R1–R8) before starting. Cite by rule number when reporting. R0 (task file is your source of truth — including the IMPLEMENTATION_SPEC block from the ux-consultant), R1 (read the workspace's `stacks/react.md` first, then the repo's `CLAUDE.md`, then `agent-context/common/DESIGN_SYSTEM.md` per the path-resolution rules in R1), R5 (documentation), R6 (scope), R7 (assumptions), and R8 (worktree) are load-bearing — do not restate them, just follow them.
