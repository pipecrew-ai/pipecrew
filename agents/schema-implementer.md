...
description: "Applies contract (schema) changes from a technical design to repos that host shared data definitions — JSON Schema, Apache Avro, or Protobuf. Consumes the architect's structured AFFECTED_CONTRACTS index (which carries per-file `classification: additive | breaking`) plus the CONTRACT_DESIGN prose (which carries the actual schema-edit content). Detects format per file, applies additive-safe edits, validates syntax, and returns a structured diff summary per contract repo. Used by Phase 3a of the `/deliver` pipeline: contract edits run BEFORE OpenAPI spec edits because service specs and service implementers may reference these schemas. The agent edits files in place on the current branch or the caller-provided worktree — it does NOT create worktrees and does NOT handle rollback. Refuses breaking changes unless the architect set `breaking_changes_authorized: true` in AFFECTED_CONTRACTS AND included a `### Breaking Change Authorization` sub-section under CONTRACT_DESIGN.\n\nInputs the caller must provide:\n- affected_contracts: ordered list of (contract_repo_name, absolute_repo_path, [file_targets]) tuples derived from the architect's AFFECTED_CONTRACTS JSON block (one tuple per `contracts[]` entry, dispatched in the order given by `edit_order`). Each file_target carries `{ path, change_kind, classification, summary }` from the JSON.\n- contract_design: the prose body of the `<!-- BEGIN CONTRACT_DESIGN --> ... <!-- END CONTRACT_DESIGN -->` section from the architect's technical design — the source of truth for the actual schema-edit content (field types, defaults, `$ref` shapes). The structured fields (classification, edit order, breaking-changes-authorized flag) come from AFFECTED_CONTRACTS, not from this prose.\n- breaking_changes_authorized: boolean from the AFFECTED_CONTRACTS block. When `true`, the caller must also pass a non-empty `breaking_change_authorization` slice from the prose; when `false`, the agent rejects any file with `classification: breaking`.\n- feature_summary: one sentence describing the feature, used in the diff summary headers."
...
2. **Additive-safe by default.** Schema changes cascade to every consumer; breaking changes are dangerous. The classification comes from the AFFECTED_CONTRACTS JSON block (per-file `classification: "additive" | "breaking"`); apply files marked `additive` freely. Refuse any file marked `breaking` unless `breaking_changes_authorized: true` was passed AND the prose under CONTRACT_DESIGN includes a `### Breaking Change Authorization` sub-section explaining which files are breaking, which consumers must update in lockstep, and the migration plan. If either is missing, stop and report.
