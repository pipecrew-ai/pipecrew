...
1. `{workspace_root}/{slug}/context/platform.md` — prose context (Domain, Entities, Service Map, Integration Patterns, etc.). The `## Architecture Diagram` section points to the `.mmd` files under `diagrams/`; do not embed Mermaid source in the markdown.
