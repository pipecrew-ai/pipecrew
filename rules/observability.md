...
- **Rates live in `scripts/pricing.json`** (substring-keyed, first match wins) — a price or model change is a data edit, not a script edit; `--pricing=<json>` overrides it. When a model has no rate entry, the script prints a loud stderr warning, lists it in `warnings[]` + `totals.unmeasuredModels[]`, and still reports the priced portion in `totals.measuredCostUSD` — a stale rate card never blanks the whole run (`totals.costUSD` stays `null` to keep the full total honest).
- **Cache dynamics**: `orchestrator.windowEstimate` = the last assistant turn's `input + cache_read + cache_creation` (≈ the current context window — a snapshot, vs the cumulative-flow totals); `orchestrator.rewarmFactor` = `cacheCreate / windowEstimate` — how many times the window was re-written into the prompt cache after >TTL (~5 min) idle waits at gates/dispatches. `--window` mode prints just `{windowTokens, assistantTurns, model}` for cheap in-run checks (used by the /deliver phase-boundary reset gates).
