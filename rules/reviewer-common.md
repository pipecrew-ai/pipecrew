...
These rules compose on top of each reviewer's stack-specific Step 5. Where a shared rule conflicts with a repo-specific convention surfaced in `AGENTS.md` or `agent-context/`, the repo convention wins for that repo — but in practice they should agree.
...
1. **Review against the repo's actual conventions**, not generic stack best practices. Read `AGENTS.md` and the agent-context docs it points to before forming any opinion. If the repo pins a library, a DI style, an error-handler shape, or a test framework, enforce those — do not substitute your own preferences. **Freshly-learned conventions are first-class rules** (see Invariant 8): a pattern the team taught the pipeline last week via `/learn` lives in `platform.md § Established Patterns` and the repo `agent-context/` — it is as binding as any older convention. A convention being recently-written is not grounds to treat it as optional.
...
### Hard checks (non-droppable)

Two checks are **deterministic and non-droppable** — the reviewer may not downgrade them to a "false positive" on judgment. They closed real escapes where a working-but-wrong change shipped past the gate.

- **HC-1 — Entity changed ⇒ migration exists (ORM stacks).** For any modified ORM model in the diff (JPA `@Entity`, Django model, SQLAlchemy / SQLModel model), every **schema-affecting change** — nullability flip, column name, length/precision, added/removed column, type change — MUST have a corresponding **new migration file included in the diff** (Liquibase changeset registered in the master changelog, Flyway versioned script, Django migration, Alembic revision). If the model changed and no matching migration is present, it is a **Critical** finding the reviewer is **not permitted to dismiss as a false positive**. The *only* valid "no migration needed" outcome is when the reviewer can point to the **existing** changeset/migration that already matches the new mapping (e.g. the column is already nullable) — and it must **cite that file:line**. "Probably fine" / "the column likely already allows this" is not an acceptable resolution. When uncertain, raise the Critical.

- **HC-2 — Contract-shape divergence from the nearest sibling needs a justification.** When the diff introduces or changes a request/response contract (endpoint shape, who-resolves-what, sync vs async, pagination source, an identifier the client round-trips), check it against the **nearest existing feature that solves a structurally similar problem** in the same repo. If the new contract shape diverges from that sibling and the architect's design (`ARCHITECTURE_DECISION`) records **no "why different" justification**, raise it: **Non-critical** for cosmetic divergence, **Critical** when the divergence adds a redundant round-trip / query or contradicts a documented established pattern. A divergence *with* a sound recorded justification is accepted — name it as a Suggestion at most. See the R10 row and Step 8 for how this composes with the inherit-don't-invent pass.
...
- **R1** Read AGENTS.md first — runtime ordering, not visible in the diff.
...
1. **Read `AGENTS.md`** (the canonical context file; if a legacy workspace has only `{repo_path}/CLAUDE.md`, read that instead), the repo's conventions docs (and `agent-context/` if pointed to), AND the workspace `platform.md § Established Patterns` (path from the dispatch). Per Invariant 8, build a checklist from the applicable established patterns + repo conventions — you will confirm the diff complies with each in Step 8.
...
4. **Contract compliance pass — depends on `spec_policy` from the dispatch.** The dispatch's `## Contract inputs` block sets `spec_policy: <api-first|code-first|no-api|infra>`. Apply the matching directive:
   ...
   - **`infra`** (CDK / Terraform; per-repo `infrastructure_impact` entry provided) — walk every resource declared in the diff against the entry's `resources_added/modified/removed` lists. Resources in code but not in the block = Critical (scope violation OR missed contract); resources in the block but not in code = Critical (incomplete). Walk every `cross_stack_refs[]` entry — both producer (`Stack.exportValue` / `CfnOutput` / Terraform `output {}`) and consumer (`Fn.importValue` / `terraform_remote_state`) sides must be present. Missing the producing-side declaration when a sibling stack imports it = Critical (deploy will fail). DO NOT flag "missing HTTP status codes", "missing request body validation", or "missing event schema files" — those concepts don't apply to infra.
5. **Stack-specific patterns** — see the per-stack reviewer's body. Consult the matching `{plugin_dir}/anti-patterns/{type}.md` catalog and flag any match in the diff. **Apply HC-1 (entity-changed ⇒ migration exists) here** for any ORM stack — it is non-droppable (see Hard checks above).
...
8. **Pattern adherence pass (R10 enforcement)** — see the implementer-common-rules table above. Walk new files / non-trivial new code blocks and ask: does an analogous existing file in this repo use the same pattern? Apply the severity grading in the table. **Two extras fold in here:** (a) confirm the diff complies with each applicable established-pattern / convention checklist item built in Step 1 (Invariant 8) — cite the complying file:line or raise a finding; (b) apply **HC-2 (contract-shape divergence from the nearest sibling)** — if a new/changed contract shape diverges from the closest analogous feature with no recorded justification in `ARCHITECTURE_DECISION`, raise it per the Hard checks grading.
...
10. **Documentation update check (R5 enforcement)** — if Step 5 or Step 8 surfaced a new pattern / new library / new architectural decision / new top-level directory, walk the diff for an accompanying doc update (`AGENTS.md`, `agent-context/`, `DESIGN_SYSTEM.md` for frontend, `platform.md § Architect Guidance` for workspace-level decisions). Missing doc update → **Non-critical**. Use the severity in the table above.
