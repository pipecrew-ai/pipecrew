...
1. **Read `CLAUDE.md`**, the repo's conventions docs (and `agent-context/` if pointed to), AND the workspace `platform.md § Established Patterns` (path from the dispatch). Per Invariant 8, build a checklist from the applicable established patterns + repo conventions — you will confirm the diff complies with each in Step 8.
