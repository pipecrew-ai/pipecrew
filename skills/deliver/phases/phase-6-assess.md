...
Launch the `{slug}-assessor` agent (published by onboarding Phase C Step 3 to the harness user-level agents dir — `~/.claude/agents/` under Claude Code, `~/.cursor/agents/` under Cursor). If it does not exist, fall back to `subagent_type: general-purpose` with a preamble that reads `{workspace_root}/{slug}/agents/assessor.md` — and log a warning prompting `/discover --resume --workspace={slug}` to publish workspace agents.
