...
`{run_dir}` = `{workspace_root}/{slug}/runs/deliver/{run_id}/` already sits in the permanent location. The timestamp prefix of `{run_id}` makes it chronologically sortable. Listing `runs/deliver/` in reverse-sort gives the history view for free. The `scratchpad.md` in the dir carries `Status: COMPLETED`, and checkpoints.jsonl has a final `run_end` event with `status: "completed"` and `duration_ms` — the reporter and any cross-run analysis tool can distinguish finished from in-flight by either signal.
