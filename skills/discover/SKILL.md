...
| `--auto-agents` | In Phase C Step 3.25, skip the interactive gate and automatically generate workspace-local implementers for every unsupported type — the non-interactive / CI path. Equivalent to choosing **Generate** for every type without prompting. Also auto-generates paired reviewers only if `--auto-reviewers` is also passed (reviewer generation is always opt-in). Default (no flag) = interactive gate. |
