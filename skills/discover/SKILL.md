...
| `--no-cache` | Force a fresh scan of every repo in Phase B2.0 — treats every cache entry as a miss. Use when you've made repo changes the cache can't detect (e.g., test fixture edits that don't move `HEAD`). The cache is still updated after the run so the next invocation benefits. |
