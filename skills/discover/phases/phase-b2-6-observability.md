...
**Skip if**: the workspace has no repo with `role: "infrastructure"` AND no `mock-server` repo with a `docker-compose.yml`. In that case write an empty sidecar `{workspace_root}/{slug}/context/observability.json` with the contents `{"log_destinations": [], "trace": {}, "dashboards": [], "runbooks": {}}` **using the Write tool — not a shell `echo`/heredoc.** Brace-plus-quote JSON on the Bash command line trips Claude Code's command-safety classifier ("contains brace with quote character") and prompts for approval needlessly; the Write tool writes the same bytes with no prompt. Then substitute the `{{OBSERVABILITY_PROSE}}` placeholder in platform.md with a one-line "no infra repo — routing table empty" note, and proceed to B3 — the troubleshooter still works (it'll ask the user to paste logs) but its routing table is empty.
