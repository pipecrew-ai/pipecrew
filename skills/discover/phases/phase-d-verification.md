...
### Step 3.5: Domain-agent template-version check (soft — warn, don't fail)

Each generated domain agent carries a `<!-- pipecrew-template-version: X.Y.Z -->` stamp (the plugin version it was generated from). Compare each against the current plugin version:

```bash
CUR=$(node -e "console.log(require('{plugin_dir}/.claude-plugin/plugin.json').version)")
for a in product-owner assessor troubleshooter; do
  grep -o 'pipecrew-template-version: [0-9][^ ]*' "{workspace_root}/{slug}/agents/$a.md" 2>/dev/null || echo "pipecrew-template-version: (none) $a"
done
echo "current: $CUR"
```

- A stamp **equal** to the current version → fine (a full discovery just generated them).
- A stamp that is **missing** (agent predates stamping) or **differs** from the current version → the workspace's domain agents were generated by an older plugin and may carry stale instructions (this is the class of bug where a regenerated template fix — e.g. the troubleshooter reading the `observability.json` sidecar — never reaches an existing workspace because incremental discovery skips agent regeneration). **Warn, don't fail:**

  ```
  ⚠ Domain agents were generated by plugin v{stamp}; current is v{CUR}.
    They may be missing template fixes. Regenerate with:
      /discover --resume --workspace={slug}   (re-runs Phase C Step 3)
  ```

  In **incremental** mode this is the expected place to surface drift — Phase C deliberately skips domain-agent regeneration, so a plugin upgrade since the last full run shows up here.
