...
**Producer**: `solution-architect` (Phase 2 design output)
**Consumers**: Phase 4.5 `task-planner` (filters `components[]` / `routes[]` / `api_integration[]` per sub-task when building each frontend task file's Architecture context), Phase 5b `react-implementer` / `nextjs-implementer` (reads via the task file), Phase 5b `pipecrew:ux-consultant` (sees the structured component tree at dispatch time).
...
| `entities[]` | array\|null | `{ name, purpose, key_states[], states_source, entity_kind, owning_module }`. `null` for frontend / infra repos. `purpose`: one sentence (≤120 chars) on what the entity represents in the domain, sampled from class Javadoc / docstring; empty string when the discoverer would be guessing. `key_states`: state-field values when the entity has a clear lifecycle. `states_source` (schema_version ≥ 2, optional): `file:line` where the state enum is defined (or `null`) — lets the architect cite the source and reconcile divergence. `entity_kind` (schema_version ≥ 2, optional): `authoritative` \| `projection` \| `consumed` — when the same entity name appears in two repos with differing `key_states`, this tells the architect to render them separately instead of flattening. **Transitions are NOT extracted** — they live scattered across business logic and are out of scope for B2.0; if a lifecycle is non-trivial (4+ states), the discoverer flags it in `notes_for_architect` and the architect may spend ONE targeted read on the named service file. |
...
| `roles[]` | array\|null | (schema_version ≥ 2, optional) The code's actual role/authority enum: `{ name, source, description }` where `source` is the `file:line` of the enum value or the `@PreAuthorize`/guard it was read from. The architect builds platform.md's `## User Roles & Permissions` table from these real values (cited) instead of the product-level `config.domain.user_roles`, which routinely omits enforced roles. `[]` / `null` when the repo has no role concept. |
